...
from aqt import QTextCursor, QTextEdit, pyqtSignal
...
class ReactiveEditText(QTextEdit, ReactiveWidget, Generic[T]):
    _key: str
    onChange = pyqtSignal(str)
    _state: StateManager[T]

    def __init__(self, state: StateManager[T], key: str):
        super().__init__()
        self._key = key
        state.bind(self)
        self._state = state
