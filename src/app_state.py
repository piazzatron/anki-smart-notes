...
class AppStateManager:
    ...
    def __init__(self) -> None:
        self._state = StateManager[AppState]({"subscription": "Loading"})

    def update_subscription_state(self) -> None:
        if not config.auth_token:
            logger.debug("User is not authenticated")
            self._state.update({"subscription": "UNAUTHENTICATED"})
            return

        def on_new_status(status: SubscriptionStatus) -> None:
            logger.debug(f"Got new subscription status: {status}")
            self._state.update({"subscription": status["subscriptionState"]})
